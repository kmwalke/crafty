...
RSpec.describe User do
  ...
  it 'removes from inventory' do
    item = create(:gatherable_fruit, parent_inventory: player.child_inventory)
    player.remove_item(item)

    expect(player.child_inventory.items.include?(item)).to be false
  end
  ...
  describe 'carried items' do
    let!(:item1) { create(:gatherable_fruit, parent_inventory: player.child_inventory) }
    ...
    describe 'with vehicle' do
      ...
      # player.carried_items == player.child_inventory.items + player.vehicle.child_inventory.items
